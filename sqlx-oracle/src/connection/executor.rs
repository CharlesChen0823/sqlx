...
use crate::message::{self, BackendMessageFormat};
use crate::statement::OracleStatementMetadata;
use crate::{
    statement::OracleStatement, Oracle, OracleArguments, OracleConnection, OracleQueryResult,
    OracleRow, OracleTypeInfo, OracleValueFormat,
};
...
async fn prepare(
    ...
    sql: &str,
    parameters: &[OracleTypeInfo],
    metadata: Option<Arc<OracleStatementMetadata>>,
) -> Result<(StatementId, Arc<OracleStatementMetadata>), Error> {
    ...
    let metadata = if let Some(metadata) = metadata {
        ...
        Arc::new(OracleStatementMetadata {
            parameters,
            columns,
...
async fn recv_desc_params(conn: &mut OracleConnection) -> Result<ParameterDescription, Error> {
    conn.inner.stream.recv_expect().await
}

async fn recv_desc_rows(conn: &mut OracleConnection) -> Result<Option<RowDescription>, Error> {
...
impl OracleConnection {
    ...
    pub(crate) async fn run<'e, 'c: 'e, 'q: 'e>(
        ...
        persistent: bool,
        metadata_opt: Option<Arc<OracleStatementMetadata>>,
    ) -> Result<impl Stream<Item = Result<Either<OracleQueryResult, OracleRow>, Error>> + 'e, Error>
    {
        ...
        let mut metadata: Arc<OracleStatementMetadata>;

        let format = if let Some(mut arguments) = arguments {
            // Check this before we write anything to the stream.
            //
            // Note: Oracle actually interprets this value as unsigned,
            ...
            let num_params = u16::try_from(arguments.len()).map_err(|_| {
                err_protocol!(
                    "OracleConnection::run(): too many arguments for query: {}",
                    arguments.len()
                )
            })?;
            ...
            self.inner.stream.write_msg(Bind {
                ...
                statement,
                formats: &[OracleValueFormat::Binary],
                num_params,
                params: &arguments.buffer,
                result_formats: &[OracleValueFormat::Binary],
            })?;
            ...
            // finally, [Sync] asks oracle to process the messages that we sent and respond with
            // a [ReadyForQuery] message when it's completely done. Theoretically, we could send
            // dozens of queries before a [Sync] and oracle can handle that. Execution on the server
            ...
            metadata = Arc::new(OracleStatementMetadata::default());
            ...
            loop {
                ...
                match message.format {
                    ...
                    BackendMessageFormat::CommandComplete => {
                        ...
                        r#yield!(Either::Left(OracleQueryResult {
                            rows_affected,
                        }));
                    }
                    ...
                    BackendMessageFormat::RowDescription => {
                        ...
                        metadata = Arc::new(OracleStatementMetadata {
                            column_names: Arc::new(column_names),
                            columns,
