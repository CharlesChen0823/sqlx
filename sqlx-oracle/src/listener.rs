...
use crate::{
    Oracle, OracleConnection, OracleQueryResult, OracleRow, OracleStatement, OracleTypeInfo,
};

/// A stream of asynchronous notifications from Oracle.
///
...
pub struct OracleListener {
    pool: Pool<Oracle>,
    connection: Option<PoolConnection<Oracle>>,
...
impl OracleListener {
    ...
    pub async fn connect_with(pool: &Pool<Oracle>) -> Result<Self, Error> {
    ...
    ///
    /// Otherwise, `pool.close().await` would have to wait until `OracleListener` encountered a
    ...
    ///
    /// However, if you want `OracleListener` to ignore the close event and continue waiting for a
    /// message as long as it can, set this to `true`.
    ///
    /// Does nothing if this was constructed with [`OracleListener::connect()`], as that creates an
    /// internal pool just for the new instance of `OracleListener` which cannot be closed manually.
    ...
    #[inline]
    async fn connection(&mut self) -> Result<&mut OracleConnection, Error> {
    ...
    ///
    /// If the connection to Oracle is lost, it is automatically reconnected on the next
    ...
    ///
    /// ```rust,no_run
    /// # use sqlx::oracle::OracleListener;
    /// #
    /// # sqlx::__rt::test_block_on(async move {
    /// let mut listener = OracleListener::connect("oracle:// ...").await?;
    /// loop {
    ...
    /// # }).unwrap();
    /// ```
    pub async fn recv(&mut self) -> Result<OracleNotification, Error> {
        loop {
    ...
    /// Receives the next notification available from any of the subscribed channels.
    ///
    /// If the connection to Oracle is lost, `None` is returned, and the connection is
    ...
    /// ```
    ///
    /// [`eager_reconnect`]: OracleListener::eager_reconnect
    pub async fn try_recv(&mut self) -> Result<Option<OracleNotification>, Error> {
        ...
        loop {
            ...
            match message.format {
                ...
                BackendMessageFormat::NotificationResponse => {
                    return Ok(Some(OracleNotification(message.decode()?)));
                }
    ...
    pub fn next_buffered(&mut self) -> Option<OracleNotification> {
        if let Ok(Some(notification)) = self.buffer_rx.try_next() {
            Some(OracleNotification(notification))
        } else {
            None
        }
    }
    ...
    ///
    /// This has the same potential drawbacks as [`recv`](OracleListener::recv).
    ///
    pub fn into_stream(mut self) -> impl Stream<Item = Result<OracleNotification, Error>> + Unpin {
        Box::pin(try_stream! {
            loop {
...
impl<'c> Acquire<'c> for &'c mut OracleListener {
    type Database = Oracle;
    type Connection = &'c mut OracleConnection;
...
impl<'c> Executor<'c> for &'c mut OracleListener {
    type Database = Oracle;
    ...
    fn fetch_optional<'e, 'q, E>(self, query: E) -> BoxFuture<'e, Result<Option<OracleRow>, Error>>
    where
        'c: 'e,
        E: Execute<'q, Self::Database>,
        'q: 'e,
...
impl Debug for OracleListener {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OracleListener").finish()
    }
}

impl Debug for OracleNotification {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("OracleNotification")
