use crate::{
    Either, Oracle, OracleColumn, OracleConnectOptions, OracleConnection, OracleQueryResult,
    OracleRow, OracleTransactionManager, OracleTypeInfo,
};
...
sqlx_core::declare_driver_with_optional_migrate!(DRIVER = Oracle);

impl AnyConnectionBackend for OracleConnection {
    fn name(&self) -> &str {
        <Oracle as Database>::NAME
    }
    ...
    fn begin(
        ...
        OracleTransactionManager::begin(self, statement)
    }

    fn commit(&mut self) -> BoxFuture<'_, sqlx_core::Result<()>> {
        OracleTransactionManager::commit(self)
    }

    fn rollback(&mut self) -> BoxFuture<'_, sqlx_core::Result<()>> {
        OracleTransactionManager::rollback(self)
    }

    fn start_rollback(&mut self) {
        OracleTransactionManager::start_rollback(self)
    }

    fn get_transaction_depth(&self) -> usize {
        OracleTransactionManager::get_transaction_depth(self)
    }
    ...
    fn fetch_many<'q>(
        ...
        Box::pin(
            self.run(query, arguments, persistent, None)
                .try_flatten_stream()
                .map(
                    move |res: sqlx_core::Result<Either<OracleQueryResult, OracleRow>>| match res? {
...
impl<'a> TryFrom<&'a OracleTypeInfo> for AnyTypeInfo {
    type Error = sqlx_core::Error;

    fn try_from(oracle_type: &'a OracleTypeInfo) -> Result<Self, Self::Error> {
        Ok(AnyTypeInfo {
            kind: match &oracle_type.0 {
                OracleType::Bool => AnyTypeInfoKind::Bool,
                OracleType::Void => AnyTypeInfoKind::Null,
                OracleType::Int2 => AnyTypeInfoKind::SmallInt,
                OracleType::Int4 => AnyTypeInfoKind::Integer,
                OracleType::Int8 => AnyTypeInfoKind::BigInt,
                OracleType::Float4 => AnyTypeInfoKind::Real,
                OracleType::Float8 => AnyTypeInfoKind::Double,
                OracleType::Bytea => AnyTypeInfoKind::Blob,
                OracleType::Text | OracleType::Varchar => AnyTypeInfoKind::Text,
                _ => {
                    return Err(sqlx_core::Error::AnyDriverError(
                        format!("Any driver does not support the Oracle type {oracle_type:?}")
                            .into(),
                    ))
                }
            },
...
impl<'a> TryFrom<&'a OracleColumn> for AnyColumn {
    type Error = sqlx_core::Error;

    fn try_from(col: &'a OracleColumn) -> Result<Self, Self::Error> {
        let type_info =
...
impl<'a> TryFrom<&'a OracleRow> for AnyRow {
    type Error = sqlx_core::Error;

    fn try_from(row: &'a OracleRow) -> Result<Self, Self::Error> {
        AnyRow::map_from(row, row.metadata.column_names.clone())
    }
}

impl<'a> TryFrom<&'a AnyConnectOptions> for OracleConnectOptions {
    type Error = sqlx_core::Error;

    fn try_from(value: &'a AnyConnectOptions) -> Result<Self, Self::Error> {
        let mut opts = OracleConnectOptions::parse_from_url(&value.database_url)?;
        opts.log_settings = value.log_settings.clone();
        Ok(opts)
    }
}

fn map_result(res: OracleQueryResult) -> AnyQueryResult {
    AnyQueryResult {
