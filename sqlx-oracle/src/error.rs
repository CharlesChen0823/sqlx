...
use crate::message::{BackendMessage, BackendMessageFormat, , OracleSeverity};

/// An error returned from the Oracle database.
pub struct OracleDatabaseError(pub(crate) OraError);

// Error message fields are documented:
// https://www.postgresql.org/docs/current/protocol-error-fields.html

impl OracleDatabaseError {
    #[inline]
    pub fn severity(&self) -> OracleSeverity {
        self.0.severity()
    }
    ...
    #[inline]
    pub fn position(&self) -> Option<OracleErrorPosition<'_>> {
        self.0
            .get_raw(b'P')
            .and_then(atoi)
            .map(OracleErrorPosition::Original)
            .or_else(|| {
                let position = self.0.get_raw(b'p').and_then(atoi)?;
                let query = self.0.get(b'q')?;

                Some(OracleErrorPosition::Internal { position, query })
            })
    }
...
pub enum OracleErrorPosition<'a> {
...
impl Debug for OracleDatabaseError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_struct("OracleDatabaseError")
...
impl Display for OracleDatabaseError {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.write_str(self.message())
    }
}

impl StdError for OracleDatabaseError {}

impl DatabaseError for OracleDatabaseError {
    fn message(&self) -> &str {
        self.message()
    }
