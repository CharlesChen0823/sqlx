...
/// Implementation of [`TransactionManager`] for Oracle.
...
impl Drop for Rollback<'_> {
    fn drop(&mut self) {
        if !self.defuse {
            OracleTransactionManager::start_rollback(self.conn)
        }
    }
}

impl<'c> Rollback<'c> {
    fn new(conn: &'c mut OracleConnection) -> Self {
        Self {
            conn,
            defuse: false,
        }
