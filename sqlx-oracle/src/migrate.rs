...
fn parse_for_maintenance(url: &str) -> Result<(OracleConnectOptions, String), Error> {
    let mut options = OracleConnectOptions::from_str(url)?;
    ...
    // use `oracle` _unless_ the database is oracle, in which case, use `template1`
    // this matches the behavior of the `createdb` util
    options.database = if database == "oracle" {
        Some("template1".into())
    } else {
        Some("oracle".into())
    };
...
impl MigrateDatabase for Oracle {
...
impl Migrate for OracleConnection {
...
CREATE TABLE IF NOT EXISTS _sqlx_migrations (
    ...
    fn lock(&mut self) -> BoxFuture<'_, Result<(), MigrateError>> {
        Box::pin(async move {
            ...
            // language=SQL
            let _ = query("SELECT oracle_advisory_lock($1)")
                .bind(lock_id)
                .execute(self)
                .await?;
    ...
    fn unlock(&mut self) -> BoxFuture<'_, Result<(), MigrateError>> {
        Box::pin(async move {
            ...
            // language=SQL
            let _ = query("SELECT oracle_advisory_unlock($1)")
                .bind(lock_id)
                .execute(self)
                .await?;
...
async fn current_database(conn: &mut OracleConnection) -> Result<String, MigrateError> {
    // language=SQL
