...
use crate::{Oracle, OracleConnection, OracleTypeInfo};

use crate::type_info::OracleArrayOf;
...
//          - message::Bind needs to take a &OracleArguments and use a `write` method instead of
...
pub struct OracleArgumentBuffer {
    ...
    // Whenever an `Encode` impl encounters a `OracleTypeInfo` object that does not have an OID
    ...
    // function and can just ask oracle.
    //
...
struct Patch {
    ...
    callback: Arc<dyn Fn(&mut [u8], &OracleTypeInfo) + 'static + Send + Sync>,
}
...
/// Implementation of [`Arguments`] for Oracle.
#[derive(Default, Debug, Clone)]
pub struct OracleArguments {
    // Types of each bind parameter
    pub(crate) types: Vec<OracleTypeInfo>,
...
impl OracleArguments {
    ...
    // Apply patches
    // This should only go out and ask oracle if we have not seen the type name yet
    pub(crate) async fn apply_patches(
        &mut self,
        conn: &mut OracleConnection,
        parameters: &[OracleTypeInfo],
    ) -> Result<(), Error> {
        let OracleArgumentBuffer {
            ref patches,
            ref type_holes,
            ref mut buffer,
            ..
...
impl OracleArgumentBuffer {
    ...
    where
        F: Fn(&mut [u8], &OracleTypeInfo) + 'static + Send + Sync,
    {
        let offset = self.len();
    ...
    pub(crate) fn patch_array_type(&mut self, array: Arc<OracleArrayOf>) {
        let offset = self.len();
    ...
    fn snapshot(&self) -> OracleArgumentBufferSnapshot {
        let Self {
            buffer,
            count,
            patches,
...
struct OracleArgumentBufferSnapshot {
    buffer_length: usize,
    count: usize,
...
impl Deref for OracleArgumentBuffer {
    type Target = Vec<u8>;
...
impl DerefMut for OracleArgumentBuffer {
    #[inline]
