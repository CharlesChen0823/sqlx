...
use crate::types::numeric::{OracleNumeric, OracleNumericSign};
...
impl Type<Oracle> for BigDecimal {
    fn type_info() -> OracleTypeInfo {
        OracleTypeInfo::NUMERIC
    }
}

impl OracleHasArrayType for BigDecimal {
    fn array_type_info() -> OracleTypeInfo {
        OracleTypeInfo::NUMERIC_ARRAY
    }
}

impl TryFrom<OracleNumeric> for BigDecimal {
    type Error = BoxDynError;

    fn try_from(numeric: OracleNumeric) -> Result<Self, BoxDynError> {
        Self::try_from(&numeric)
    }
}

impl TryFrom<&'_ OracleNumeric> for BigDecimal {
    type Error = BoxDynError;

    fn try_from(numeric: &'_ OracleNumeric) -> Result<Self, Self::Error> {
        let (digits, sign, weight) = match *numeric {
            ...
            OracleNumeric::NotANumber => {
                return Err("BigDecimal does not support NaN values".into());
            }
        };

        if digits.is_empty() {
            // Oracle returns an empty digit array for 0 but BigInt expects at least one zero
            return Ok(0u64.into());
        }

        let sign = match sign {
            OracleNumericSign::Positive => Sign::Plus,
            OracleNumericSign::Negative => Sign::Minus,
        };
        ...
        for (i, &digit) in digits.iter().enumerate() {
            if !OracleNumeric::is_valid_digit(digit) {
                return Err(format!(
                    "OracleNumeric to BigDecimal: {i}th digit is out of range {digit}"
                )
                .into());
            }
        ...
        let bigint = BigInt::from_radix_be(sign, &cents, 100)
            .ok_or("OracleNumeric contained an out-of-range digit")?;

        Ok(BigDecimal::new(bigint, scale))
    }
}

impl TryFrom<&'_ BigDecimal> for OracleNumeric {
    type Error = BoxDynError;
    ...
    fn try_from(decimal: &BigDecimal) -> Result<Self, BoxDynError> {
        ...
        let base_10_len = i64::try_from(base_10.len()).map_err(|_| {
            format!(
                "BigDecimal base-10 length out of range for OracleNumeric: {}",
                base_10.len()
            )
        })?;
        ...
        Ok(OracleNumeric::Number {
            sign: sign_to_oracle(sign),
            scale,
            weight,
            digits,
        })
...
impl Encode<'_, Oracle> for BigDecimal {
    fn encode_by_ref(&self, buf: &mut OracleArgumentBuffer) -> Result<IsNull, BoxDynError> {
        OracleNumeric::try_from(self)?.encode(buf)?;

        Ok(IsNull::No)
    }

    fn size_hint(&self) -> usize {
        OracleNumeric::size_hint(self.digits())
    }
}
...
impl Decode<'_, Oracle> for BigDecimal {
    fn decode(value: OracleValueRef<'_>) -> Result<Self, BoxDynError> {
        match value.format() {
            OracleValueFormat::Binary => OracleNumeric::decode(value.as_bytes()?)?.try_into(),
            OracleValueFormat::Text => Ok(value.as_str()?.parse::<BigDecimal>()?),
        }
    }
}

fn sign_to_oracle(sign: Sign) -> OracleNumericSign {
    match sign {
        Sign::Plus | Sign::NoSign => OracleNumericSign::Positive,
        Sign::Minus => OracleNumericSign::Negative,
    }
}

#[cfg(test)]
mod bigdecimal_to_numeric {
    use super::{BigDecimal, OracleNumeric, OracleNumericSign};
    ...
    fn zero() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&zero).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 0,
                weight: 0,
                digits: vec![]
            }
    ...
    fn one() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&one).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 0,
                weight: 0,
    ...
    fn ten() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&ten).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 0,
                weight: 0,
    ...
    fn one_hundred() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&one_hundred).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 0,
                weight: 0,
    ...
    fn ten_thousand() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&ten_thousand).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 0,
                weight: 1,
    ...
    fn two_digits() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&two_digits).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 0,
                weight: 1,
    ...
    fn one_tenth() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&one_tenth).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 1,
                weight: -1,
    ...
    fn one_hundredth() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&one_hundredth).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 2,
                weight: -1,
    ...
    fn twelve_thousandths() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&twelve_thousandths).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 3,
                weight: -1,
    ...
    fn decimal_1() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&decimal).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 4,
                weight: 0,
    ...
    fn one_digit_decimal() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&one_digit_decimal).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 8,
                weight: -2,
    ...
    fn issue_423_four_digit() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&four_digit).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 0,
                weight: 0,
    ...
    fn issue_423_negative_four_digit() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&negative_four_digit).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Negative,
                scale: 0,
                weight: 0,
                digits: vec![1234]
            }
    ...
    fn issue_423_eight_digit() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&eight_digit).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Positive,
                scale: 0,
                weight: 1,
    ...
    fn issue_423_negative_eight_digit() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&negative_eight_digit).unwrap(),
            OracleNumeric::Number {
                sign: OracleNumericSign::Negative,
                scale: 0,
                weight: 1,
