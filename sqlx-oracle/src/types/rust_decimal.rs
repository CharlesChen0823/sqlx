...
impl TryFrom<&'_ OracleNumeric> for Decimal {
    type Error = BoxDynError;

    fn try_from(numeric: &'_ OracleNumeric) -> Result<Self, BoxDynError> {
        ...
        if digits.is_empty() {
            // Oracle returns an empty digit array for 0
            return Ok(Decimal::ZERO);
        }

        let scale = u32::try_from(scale)
            .map_err(|_| format!("invalid scale value for Oracle NUMERIC: {scale}"))?;
        ...
        match sign {
            OracleNumericSign::Positive => value.set_sign_positive(true),
            OracleNumericSign::Negative => value.set_sign_negative(true),
        }
...
impl From<&'_ Decimal> for OracleNumeric {
    ...
    fn from(decimal: &Decimal) -> Self {
        ...
        // Array to store max mantissa of Decimal in Oracle decimal format.
        ...
        OracleNumeric::Number {
            sign: match decimal.is_sign_negative() {
                false => OracleNumericSign::Positive,
                true => OracleNumericSign::Negative,
            },
...
impl Decode<'_, Oracle> for Decimal {
    fn decode(value: OracleValueRef<'_>) -> Result<Self, BoxDynError> {
        match value.format() {
            ...
            OracleValueFormat::Text => Ok(value.as_str()?.parse::<Decimal>()?),
        }
    }
}
...
#[cfg(test)]
mod tests {
    use super::{Decimal, OracleNumeric, OracleNumericSign};
    ...
    fn zero() {
        ...
        assert_eq!(OracleNumeric::from(&zero), OracleNumeric::ZERO,);

        assert_eq!(
            Decimal::try_from(&OracleNumeric::ZERO).unwrap(),
            Decimal::ZERO
        );
    }
    ...
    fn decimal_4() {
        ...
        let expected_numeric = OracleNumeric::Number {
            sign: OracleNumericSign::Positive,
            scale: 5,
            weight: 1,
            digits: vec![1, 2345, 6789],
        };
        assert_eq!(OracleNumeric::try_from(&decimal).unwrap(), expected_numeric);
    ...
    fn max_value() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&Decimal::MAX).unwrap(),
            expected_numeric
        );
    ...
    fn max_value_max_scale() {
        ...
        assert_eq!(
            OracleNumeric::try_from(&max_value_max_scale).unwrap(),
            expected_numeric
        );
