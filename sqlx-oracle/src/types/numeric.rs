...
use crate::OracleArgumentBuffer;

/// Represents a `NUMERIC` value in the **Oracle** wire protocol.
#[derive(Debug, PartialEq, Eq)]
pub(crate) enum OracleNumeric {
    /// Equivalent to the `'NaN'` value in Oracle. The result of, e.g. `1 / 0`.
    NotANumber,
    ...
    /// https://github.com/oracle/oracle/blob/bcd1c3630095e48bc3b1eb0fc8e8c8a7c851eba1/src/backend/utils/adt/numeric.c#L224-L269
    Number {
        /// The sign of the value: positive (also set for 0 and -0), or negative.
        sign: OracleNumericSign,
        ...
        /// `weight`--the comment in the Oracle code linked above recommends using this only for
...
// https://github.com/oracle/oracle/blob/bcd1c3630095e48bc3b1eb0fc8e8c8a7c851eba1/src/backend/utils/adt/numeric.c#L167-L170
...
/// Possible sign values for [OracleNumeric].
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
#[repr(u16)]
pub(crate) enum OracleNumericSign {
    Positive = SIGN_POS,
    Negative = SIGN_NEG,
}

impl OracleNumericSign {
    fn try_from_u16(val: u16) -> Result<Self, BoxDynError> {
        match val {
            SIGN_POS => Ok(OracleNumericSign::Positive),
            SIGN_NEG => Ok(OracleNumericSign::Negative),

            SIGN_NAN => unreachable!("sign value for NaN passed to OracleNumericSign"),

            _ => Err(format!("invalid value for OracleNumericSign: {val:#04X}").into()),
        }
    }
}
...
impl OracleNumeric {
    ...
    pub const ZERO: Self = OracleNumeric::Number {
    ...
    pub(crate) fn decode(mut buf: &[u8]) -> Result<Self, BoxDynError> {
        // https://github.com/oracle/oracle/blob/bcd1c3630095e48bc3b1eb0fc8e8c8a7c851eba1/src/backend/utils/adt/numeric.c#L874
    ...
    pub(crate) fn encode(&self, buf: &mut OracleArgumentBuffer) -> Result<(), String> {
        match *self {
            OracleNumeric::Number {
                ...
                let digits_len = i16::try_from(digits.len()).map_err(|_| {
                    format!(
                        "OracleNumeric digits.len() ({}) should not overflow i16",
                        digits.len()
                    )
                })?;
                ...
                for (i, &digit) in digits.iter().enumerate() {
                    if !Self::is_valid_digit(digit) {
                        return Err(format!("{i}th OracleNumeric digit out of range: {digit}"));
                    }
