...
impl TestSupport for Oracle {
    ...
    fn cleanup_test_dbs() -> BoxFuture<'static, Result<Option<usize>, Error>> {
        Box::pin(async move {
            ...
            let mut conn = OracleConnection::connect(&url).await?;
...
async fn test_context(args: &TestArgs) -> Result<TestContext<Oracle>, Error> {
    let url = dotenvy::var("DATABASE_URL").expect("DATABASE_URL must be set");

    let master_opts = OracleConnectOptions::from_str(&url).expect("failed to parse DATABASE_URL");

    let pool = PoolOptions::new()
        // Oracle' normal connection limit is 100 plus 3 superuser connections
    ...
    conn.execute(
        ...
        r#"
        select oracle_advisory_xact_lock(8318549251334697844);
    ...
    let db_name = Oracle::db_name(args);
    do_cleanup(&mut conn, &db_name).await?;
...
async fn do_cleanup(conn: &mut OracleConnection, db_name: &str) -> Result<(), Error> {
